...
def semicircular(center_x: float, center_y: float, radius: float, n_points: int = 48,
                 offset_x: float = 0, offset_y: float = 0):
    """
    Galería semicircular: base plana en y = center_y y techo en arco de radio 'radius'.
    Devuelve una polilínea CERRADA, orden horario:
    izquierda base -> arco superior -> derecha base -> (cierre a izquierda base).
    """
    # centro actualizado
    ...
    # 1) Punto de inicio: izquierda de la base
    verts.append((cx - radius, cy))

    # 2) Arco superior: de 0..pi en sentido antihorario,
    #    pero parametrizado para recorrer de izquierda (-R,cy) a derecha (+R,cy)
    #    sin repetir los puntos de la base.
    #    Usamos i=1..n_points-1 para no duplicar extremos.
    for i in range(1, n_points):
        theta = pi * i / n_points  # 0 < theta < pi
        x = cx + radius * cos(theta - pi)   # cos(theta - pi) = -cos(theta)
        y = cy + radius * sin(theta)
        verts.append((x, y))

    # 3) Derecha de la base
    verts.append((cx + radius, cy))

    # 4) Cierre: volver al primer vértice
    if verts[-1] != verts[0]:
