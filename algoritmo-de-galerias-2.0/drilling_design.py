...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self.header.grid(row=0, column=0, columnspan=2,       # ocupa el ancho completo
        ...
        tab_eje     = ttk.Frame(self.tabs); self.tabs.add(tab_eje,     text="Eje")
        tab_geom    = ttk.Frame(self.tabs); self.tabs.add(tab_geom,    text="Galería")
        tab_cuele   = ttk.Frame(self.tabs); self.tabs.add(tab_cuele,   text="Cuele")
        tab_cc      = ttk.Frame(self.tabs); self.tabs.add(tab_cc,      text="Contracuele")
        tab_zap     = ttk.Frame(self.tabs); self.tabs.add(tab_zap,     text="Zapateras")
        tab_cajas   = ttk.Frame(self.tabs); self.tabs.add(tab_cajas,   text="Cajas")
        tab_corona  = ttk.Frame(self.tabs); self.tabs.add(tab_corona,  text="Corona")
        tab_aux     = ttk.Frame(self.tabs); self.tabs.add(tab_aux,     text="Auxiliares")
       ...
       # --- Interacción normal con botón izquierdo ---
        ...
        self.canvas.event_add("<<PanEnd>>",   "<ButtonRelease-2>", "<Shift-ButtonRelease-1>")
        ...
        self._vbind(self, "<<DelKey>>", self._delete_selected, "<Delete>", "<BackSpace>")
        ...
        ttk.Checkbutton(opts, text="Mostrar ejes X/Y", variable=self.show_axes,
                        command=self.draw).pack(anchor="w")
        ttk.Checkbutton(opts, text="Bloquear creación (evitar nueva galería por click)",
        ...
        ttk.Label(self.side, text="Arrastra perforaciones para ajustarlas manualmente.")\
    ...
    def _on_tab_changed(self, ev):
        ...
        if not self._can_go_to(idx):
            self._sync_tabs()
            messagebox.showinfo("Paso bloqueado", "Completa el paso anterior antes de continuar.")
            return
        self.step = idx
        self._render_step_panel()
        self._update_step_label()
    ...
    def _render_step_panel(self):
        ...
        if self.step == SP_EJE:
            ...
            ttk.Button(frm, text="Centrar en el canvas", command=self._center_origin).pack(anchor="w")
            ttk.Button(self.step_frame, text="Borrar este paso",
                    command=lambda: self._clear_step(SP_EJE)).pack(anchor="w", pady=(6,0))
    ...
    def next_step(self):
        ...
        if   self.step == SP_EJE    and not self.done_eje:    return messagebox.showwarning("Eje", "Fija el origen con un click o pulsa 'Centrar'.")
        elif self.step == SP_GEOM   and not self.done_geom:   return messagebox.showwarning("Geometría", "Coloca la geometría con un click en el canvas.")
        elif self.step == SP_CUELES and not self.done_cueles: return messagebox.showwarning("Cueles", "Inserta al menos un cuele.")
        elif self.step == SP_CC     and not self.done_cc:     return messagebox.showwarning("Contracuele", "Inserta el contracuele (click o doble click).")
        elif self.step == SP_ZAP    and not self.done_zap:    return messagebox.showwarning("Zapateras", "Pulsa 'Agregar' para colocar las zapateras.")
        elif self.step == SP_CAJAS  and not self.done_cajas:  return messagebox.showwarning("Cajas", "Pulsa 'Agregar' para colocar las cajas.")
        elif self.step == SP_CORONA and not self.done_corona: return messagebox.showwarning("Corona", "Pulsa 'Agregar' para colocar la corona.")
    ...
    def on_click(self, ev):
        ...
        if (self.step == SP_GEOM and self.lock_geom_insert.get() and
            (self.drift_poly or self.scene.drifts)):
            return
    ...
    def _do_cajas(self):
        ...
        holes = place_cajas(
            ...
            top_clear_m=0.1, bottom_clear_m=0.05
        )
        if not holes:
    ...
    def _clear_step(self, step_to_clear):
        """
        Borra el contenido generado en 'step_to_clear'.
        - Si borras GEOM, se reinicia TODO el flujo (excepto el paneo).
        - Para el resto de pasos, sólo se eliminan las perforaciones de ese paso
        y se bajan las banderas correspondientes.
        """
        # --- 1) Si se borra la geometría, resetea todo ---
        # (Si más adelante agregas SP_AXIS como paso 0, cambia la condición por:
        #    if step_to_clear in (SP_GEOM, SP_AXIS):  )
        if step_to_clear == SP_GEOM:
            ...
            # Reset de límites de pared (para CAJAS)
            self.wall_top_y = None
            self.wall_x_left = None
            self.wall_x_right = None
            ...
            # Volver al paso de geometría (o al de Eje si lo agregas)
            self.step = SP_GEOM  # si agregas SP_AXIS, usa: self.step = SP_AXIS
        ...
        # --- 2) Para cualquier otro paso: elimina sólo lo de ese paso ---
        self.scene.remove_holes_by_step(step_to_clear)

        # Limpia selección/drag por si apuntaban a items borrados
        self.scene.selected_idx = None
        self.dragging_idx = None

        # --- 3) Baja la bandera del paso borrado ---
        ...
        # --- 4) Redibuja y refresca el panel del paso actual ---
        self.draw()
        self._render_step_panel()
        self._update_step_label()
    ...
    def clear_all(self):
        ...
        # flags
        self.step = SP_EJE
        self.done_eje = self.done_geom = self.done_zap = self.done_cajas = False
